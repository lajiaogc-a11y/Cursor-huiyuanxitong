...
export const TABLE_SCHEMAS: Record<string, string> = {
  ...
  navigation_config: `CREATE TABLE IF NOT EXISTS public.navigation_config (
  id uuid DEFAULT gen_random_uuid() NOT NULL PRIMARY KEY,
...
-- NOTE: These functions use Supabase-specific features (extensions.crypt).
-- For non-Supabase PostgreSQL, replace 'extensions.crypt' with 'crypt' 
-- and 'extensions.gen_salt' with 'gen_salt' (requires pgcrypto extension).
...
-- Password hashing trigger (Supabase version uses extensions.crypt)
-- For standard PostgreSQL, change extensions.crypt -> crypt, extensions.gen_salt -> gen_salt
...
- PostgreSQL 14 或更高版本（推荐使用 Supabase 托管 PostgreSQL）
- 有创建数据库和表的权限
- psql 命令行工具 或 Supabase SQL Editor

## 方式一：导入到 Supabase（推荐）

### 步骤 1：创建新的 Supabase 项目
在 https://supabase.com 创建一个新项目。
...
2) 02_functions.sql — 创建数据库函数和触发器
   ⚠️ 注意：Supabase 中需将 crypt() → extensions.crypt()，gen_salt() → extensions.gen_salt()
3) 05_data.sql    — 导入数据（使用 ON CONFLICT DO NOTHING 防止重复）
4) 04_indexes.sql — 创建索引（可选，提升查询性能）
5) 03_policies.sql — 配置 RLS 策略（仅 Supabase 需要）
\`\`\`
...
CREATE EXTENSION IF NOT EXISTS pgcrypto;
\`\`\`
在 Supabase 中，函数使用 extensions.crypt() 而非 crypt()。
