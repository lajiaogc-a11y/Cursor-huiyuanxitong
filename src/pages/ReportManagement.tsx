...
export default function ReportManagement() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const live = await loadSharedData<{ recommended?: { mid?: number } }>('usdtLiveRates');
        if (live?.recommended?.mid && live.recommended.mid > 0) {
          setUsdtRateForReport(live.recommended.mid);
          return;
        }
        const calc = await loadSharedData<{ rate?: number }>('calculatorUsdtRate');
        if (calc?.rate && calc.rate > 0) {
          setUsdtRateForReport(calc.rate);
        }
      } catch (_) {}
    };
    load();
  ...
  const employeeProfitData = useMemo<EmployeeProfitData[]>(() => {
    ...
    const allNgnGhsPositiveProfit = activeOrders
      .filter((o) => o.currency !== "USDT" && (Number(o.profit_ngn) || 0) > 0)
      .reduce((sum, o) => sum + (Number(o.profit_ngn) || 0), 0);
    const allUsdtPositiveProfit = activeOrders
      .filter((o) => o.currency === "USDT" && (Number(o.profit_usdt) || 0) > 0)
      .reduce((sum, o) => sum + (Number(o.profit_usdt) || 0), 0);
    const totalGlobalProfit = allNgnGhsPositiveProfit + (allUsdtPositiveProfit * usdtRate);
    ...
    return employees.map((emp) => {
      ...
      // 利润(NGN/GHS) = 赛地/奈拉模式中利润 > 0 的订单总和
      const profitNgn = ngnGhsOrders
        .filter((o) => (Number(o.profit_ngn) || 0) > 0)
        .reduce((sum, o) => sum + (Number(o.profit_ngn) || 0), 0);
      
      // 利润(USDT) = USDT模式中利润 > 0 的订单总和
      const profitUsdt = usdtOrders
        .filter((o) => (Number(o.profit_usdt) || 0) > 0)
        .reduce((sum, o) => sum + (Number(o.profit_usdt) || 0), 0);
      
      // 错单(NGN/GHS) = 赛地/奈拉模式中利润 < 0 的订单总和
      const errorProfitNgn = ngnGhsOrders
        .filter((o) => (Number(o.profit_ngn) || 0) < 0)
        .reduce((sum, o) => sum + (Number(o.profit_ngn) || 0), 0);
      
      // 错单(USDT) = USDT模式中利润 < 0 的订单总和
  ...
  const cardReportData = useMemo<CardReportData[]>(() => {
    ...
    return orderTypes.map((cardId) => {
      ...
      const profitNgn = matchedOrders
        .filter((o) => o.currency !== "USDT")
        .reduce((sum, o) => sum + (Number(o.profit_ngn) || 0), 0);
      const profitUsdt = matchedOrders
        .filter((o) => o.currency === "USDT")
        .reduce((sum, o) => sum + (Number(o.profit_usdt) || 0), 0);
  ...
  // ============= 卡商报表 =============
  // 按 card_merchant_id 统计每个卡商的核销面值总额
  // 统计规则：赛地/奈拉模式 + USDT模式下，该卡商在订单管理中的卡价值（amount字段）总和
  // 员工只能看到自己的订单数据
  const vendorReportData = useMemo<VendorReportData[]>(() => {
    ...
    // 获取所有出现的 card_merchant_id（卡商ID）
    const vendorIds = [...new Set(filteredOrders.map(o => o.card_merchant_id).filter(Boolean))];
    
    return vendorIds.map((vendorId) => {
      // 按 card_merchant_id 统计，不按名称
      const matchedOrders = filteredOrders.filter(
        (o) => o.card_merchant_id === vendorId && o.status === "completed"
      );
      
      const orderCount = matchedOrders.length;
      // 核销面值总额 = 赛地/奈拉模式 + USDT模式的卡价值（amount字段）总和
      const cardValueSum = matchedOrders.reduce(
        (sum, o) => sum + (Number(o.amount) || 0),
        0
      );
      ...
      return {
        vendorId: vendorId,
        vendorName: vendorId, // 存储card_merchant_id，显示时通过resolveVendorName解析为名称
        orderCount,
        cardValueSum,
        profitNgn,
        profitUsdt,
  ...
  // ============= 代付报表 =============
  // 按 vendor_id (代付商家ID) 统计每个代付商家的订单数量和代付总额
  // 员工只能看到自己的订单数据
  const paymentProviderReportData = useMemo<PaymentProviderReportData[]>(() => {
    ...
    // 获取所有出现的 vendor_id（代付商家ID）
    const providerIds = [...new Set(filteredOrders.map(o => o.vendor_id).filter(Boolean))];
    
    return providerIds.map((providerId) => {
      // 按 vendor_id 统计已完成订单
      const matchedOrders = filteredOrders.filter(
        (o) => o.vendor_id === providerId && o.status === "completed"
      );
      ...
      return {
        ...
        providerName: providerId, // 存储vendor_id，显示时通过resolveProviderName解析为名称
        orderCount,
        paymentValueNgnGhs,
        paymentValueUsdt,
      };
  ...
  // 统计规则：
  // - 卡价值总额 = 赛地/奈拉模式 + USDT模式的卡价值(amount字段)总和
  // - 代付价值（奈赛）总和 = 赛地/奈拉模式的代付价值总和
  // - 代付价值USDT总和 = USDT模式代付价值总和
  // - 活动发放 = 会员管理中活动赠送的赠送价值(gift_value字段)总和
  // - 利润（人）= 奈拉 + 赛地模式本单利润总和
  // - 利润（USDT）= USDT模式本单利润总和
  // - 总利润（人）= 卡价值总额 - 代付价值（奈赛）总和 - 代付价值USDT总和 × USDT汇率 - 活动发放
  // 员工只能看到自己的订单数据
  const dailyReportData = useMemo<DailyReportData[]>(() => {
    ...
    return Array.from(dateMap.values()).map(data => ({
      ...data,
      // 总利润(人) = 卡价值总额 - 代付价值（奈赛）总和 - 代付价值USDT总和 × USDT汇率 - 活动发放
      totalProfit: data.cardValueSum - data.paymentValueNgnGhs - (data.paymentValueUsdt * usdtExchangeRate) - data.activityAmount,
    })).sort((a, b) => {
  ...
  const providerSummary = useMemo(() => {
    const data = paymentProviderReportData.filter(item => 
      (item.providerName ?? '').toLowerCase().includes(searchLowerSafe)
    );
    return {
      orderCount: data.reduce((sum, d) => sum + d.orderCount, 0),
  ...
  const dailySummary = useMemo(() => {
    const data = dailyReportData.filter(item => (item.date ?? '').includes(searchTerm));
    return {
      orderCount: data.reduce((sum, d) => sum + d.orderCount, 0),
      cardValueSum: data.reduce((sum, d) => sum + d.cardValueSum, 0),
      paymentValueNgnGhs: data.reduce((sum, d) => sum + d.paymentValueNgnGhs, 0),
  ...
  const monthlySummary = useMemo(() => {
    const data = monthlyReportData.filter(item => (item.month ?? '').includes(searchTerm));
    return {
      orderCount: data.reduce((sum, d) => sum + d.orderCount, 0),
      cardValueSum: data.reduce((sum, d) => sum + d.cardValueSum, 0),
      paymentValueNgnGhs: data.reduce((sum, d) => sum + d.paymentValueNgnGhs, 0),
  ...
  const handleExport = () => {
    ...
    switch (activeTab) {
      ...
      case 'vendor': {
        ...
        exportToCSV(data, [
          { key: 'vendorName', label: '卡商名称', labelEn: 'Vendor', formatter: (v) => resolveVendorName(v) },
          { key: 'orderCount', label: '订单数量', labelEn: 'Orders' },
      ...
      case 'provider': {
        ...
        exportToCSV(data, [
          { key: 'providerName', label: '商家名称', labelEn: 'Provider', formatter: (v) => resolveProviderName(v) },
          { key: 'orderCount', label: '订单数量', labelEn: 'Orders' },
      ...
      case 'activity': {
        const data = activityReportData.filter(item => 
          (item.activityTypeLabel ?? '').toLowerCase().includes(searchLowerSafe) || (item.date ?? '').includes(searchTerm)
        );
        exportToCSV(data, [
          { key: 'date', label: '日期', labelEn: 'Date' },
    ...
    switch (activeTab) {
      ...
      case 'vendor':
        ...
        rows = vendorReportData.map(item => [
          resolveVendorName(item.vendorName), item.orderCount, formatNumber(item.cardValueSum),
          formatNumber(item.profitNgn), formatNumber(item.profitUsdt)
        ]);
        title = '卡商报表';
        break;
      case 'provider':
        ...
        rows = paymentProviderReportData.map(item => [
          resolveProviderName(item.providerName), item.orderCount,
  ...
  // 排序状态
  const [employeeSort, setEmployeeSort] = useState<SortConfig | null>(null);
  const [cardSort, setCardSort] = useState<SortConfig | null>(null);
  const [vendorSort, setVendorSort] = useState<SortConfig | null>(null);
  const [providerSort, setProviderSort] = useState<SortConfig | null>(null);
  const [dailySort, setDailySort] = useState<SortConfig | null>(null);
  const [monthlySort, setMonthlySort] = useState<SortConfig | null>(null);
  const [activitySort, setActivitySort] = useState<SortConfig | null>(null);
  ...
  const { sortedData: sortedProviderData, requestSort: requestProviderSort, sortConfig: providerSortConfig } = 
    useSortableData(paymentProviderReportData.filter(item => (item.providerName ?? '').toLowerCase().includes(searchLower)), providerSort);
  const { sortedData: sortedDailyData, requestSort: requestDailySort, sortConfig: dailySortConfig } = 
    useSortableData(dailyReportData.filter(item => (item.date ?? '').includes(searchTerm)), dailySort);
  const { sortedData: sortedMonthlyData, requestSort: requestMonthlySort, sortConfig: monthlySortConfig } = 
    useSortableData(monthlyReportData.filter(item => (item.month ?? '').includes(searchTerm)), monthlySort);
  const { sortedData: sortedActivityData, requestSort: requestActivitySort, sortConfig: activitySortConfig } = 
    useSortableData(activityReportData.filter(item => (item.activityTypeLabel ?? '').toLowerCase().includes(searchLower) || (item.date ?? '').includes(searchTerm)), activitySort);
    ...
    return (
      <div className="flex items-center justify-between py-2 px-1 border-t">
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">{t("每页", "Per page")}</span>
          ...
          <span className="text-xs text-muted-foreground">{t("条", "items")}</span>
          <span className="text-xs text-muted-foreground ml-2">
            {t(`显示 ${startItem}-${endItem} / 共 ${totalItems} 条`, `Showing ${startItem}-${endItem} of ${totalItems}`)}
          </span>
        </div>
                ...
                {useCompactLayout ? (
                  <MobileCardList>
                    {paginatedEmployeeData.length === 0 ? (
                      <p className="text-center py-8 text-muted-foreground text-sm">{t("暂无数据", "No data")}</p>
                    ) : paginatedEmployeeData.map((item) => (
                      <MobileCard key={item.employeeId}>
                        <MobileCardHeader>
                    ...
                    <TableBody>
                      {paginatedVendorData.map((item) => (
                          <TableRow key={item.vendorId}>
                            <TableCell className="text-center font-medium px-1.5">{resolveVendorName(item.vendorName)}</TableCell>
                            <TableCell className="text-center px-1.5">{item.orderCount}</TableCell>
                            <TableCell className="text-center px-1.5">{formatNumber(item.cardValueSum)}</TableCell>
                            <TableCell className="text-center px-1.5">{formatNumber(item.profitNgn)}</TableCell>
                            <TableCell className="text-center px-1.5">{formatNumber(item.profitUsdt)}</TableCell>
                ...
                {useCompactLayout ? (
                  <MobileCardList>
                    {paginatedProviderData.length === 0 ? (
                      ...
                      <MobileCard key={item.providerId}>
                        <MobileCardHeader>
                    ...
                    <TableBody>
                      {paginatedProviderData.map((item) => (
                          <TableRow key={item.providerId}>
                            <TableCell className="text-center font-medium px-1.5">{resolveProviderName(item.providerName)}</TableCell>
                            <TableCell className="text-center px-1.5">{item.orderCount}</TableCell>
