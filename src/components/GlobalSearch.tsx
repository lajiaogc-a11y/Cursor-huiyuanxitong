...
export function GlobalSearch() {
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      // Search members by phone or member_code
      const { data: members } = await supabase
        .from('members')
        .select('id, phone_number, member_code')
        .or(`phone_number.ilike.%${q}%,member_code.ilike.%${q}%`)
        .limit(5);
      members?.forEach(m => {
        searchResults.push({
          type: 'member',
      ...
      // Search cards (merchants) by name
        ...
        <DialogContent className="sm:max-w-[500px] p-0 gap-0">
          <div className="flex items-center border-b border-border px-3">
            ...
            <Input
              ...
              placeholder={t('搜索会员、订单、商家...', 'Search members, orders, merchants...')}
              className="border-0 focus-visible:ring-0 h-12 text-sm"
            />
            {query && (
              <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={() => { setQuery(''); setResults([]); }}>
