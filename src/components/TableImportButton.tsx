...
export default function TableImportButton({
  ...
  const [importPreview, setImportPreview] = useState<{
    headers: string[];
    rowCount: number;
    validation: { valid: boolean; errors: string[]; columnMapping: Record<string, string> } | null;
  } | null>(null);
  ...
  const handleImport = async () => {
    ...
    reader.onload = async (e) => {
      const content = e.target?.result as string;
      const result = await importTableFromCSV(
        tableName,
        content,
        isEnglish,
        importMode,
  ...
  return (
    <>
      ...
      {/* 导入预览对话框 */}
      <Dialog open={showImportDialog} onOpenChange={(open) => !open && resetState()}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('预览并确认要导入的数据', 'Preview and confirm the data to import')}
            </DialogDescription>
          </DialogHeader>
