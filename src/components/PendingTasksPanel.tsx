...
import { countUnreadNotificationsForRecipient } from "@/services/notifications/notificationService";
...
export default function PendingTasksPanel() {
  ...
  // 与审核中心 useAuditRecords 一致：平台超管仅在看租户视图时用该租户统计，避免把全库待审核算进「待办」
    ...
    const load = async () => {
      try {
        ...
        const notifPromise = employee?.id
          ? countUnreadNotificationsForRecipient(employee.id).then((count) => ({ count }))
          : Promise.resolve({ count: 0 });

        // Recent error reports (24h, admin only) — 与 SystemHealthMonitor 同源 API
        const errorPromise =
          employee?.role === "admin"
            ? countErrorReportsSince(new Date(Date.now() - 86400000).toISOString()).then((count) => ({
                count,
              }))
            : Promise.resolve({ count: 0 });
