...
import { bulkDeleteApi } from '@/services/admin/adminApiService';
...
async function loadNavConfig(): Promise<NavigationConfig[]> {
  const { getNavigationConfig } = await import('@/api/data');
...
async function saveNavConfig(config: NavigationConfig[]): Promise<void> {
  const { postNavigationConfig } = await import('@/api/data');
  const ok = await postNavigationConfig(config.map((nav) => ({
      nav_key: nav.navKey,
  ...
  if (!ok) throw new Error('save navigation config failed');
}
...
export default function DataManagementTab() {
  ...
  const handleDeleteData = async () => {
    ...
    // 验证管理员密码（通过 Backend API）
    const { verifyAdminPasswordApi } = await import('@/services/admin/adminApiService');
    ...
    try {
      ...
      const { deletedSummary, errors } = result;
      const totalCount = deletedSummary.reduce((acc, s) => acc + s.count, 0);
      const hasOrdersOrMembers = deletedSummary.some(s => s.table === '订单' || s.table === '会员');
