...
import { getOrdersReportApi } from '@/services/reports/reportsApiService';
    ...
    <div className="space-y-4 overflow-x-hidden">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-medium text-sm truncate">{t('利润对比分析', 'Profit Comparison')}</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card className="overflow-hidden">
          <CardContent className="p-2.5">
            <p className="text-[11px] text-muted-foreground truncate">{t('订单数', 'Orders')}</p>
            <div className="flex items-center gap-1 mt-1 flex-wrap">
              <span className="text-base font-bold">{current.orders}</span>
        ...
        <Card className="overflow-hidden">
          <CardContent className="p-2.5">
            <p className="text-[11px] text-muted-foreground truncate">{t('利润(元)', 'Profit(CNY)')}</p>
            <div className="flex items-center gap-1 mt-1 flex-wrap">
              <span className="text-base font-bold truncate">¥{safeToFixed(current.profitNgn, 0)}</span>
              <ChangeIndicator current={current.profitNgn} previous={previous.profitNgn} />
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5 truncate">
              {previous.label}: ¥{safeToFixed(previous.profitNgn, 0)}
            </p>
          </CardContent>
        </Card>
        <Card className="overflow-hidden">
          <CardContent className="p-2.5">
            <p className="text-[11px] text-muted-foreground truncate">{t('利润(U)', 'Profit(U)')}</p>
            <div className="flex items-center gap-1 mt-1 flex-wrap">
              <span className="text-base font-bold truncate">{safeToFixed(current.profitUsdt, 2)}</span>
              <ChangeIndicator current={current.profitUsdt} previous={previous.profitUsdt} />
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5 truncate">
              {previous.label}: {safeToFixed(previous.profitUsdt, 2)}
            </p>
          </CardContent>
        </Card>
      </div>
