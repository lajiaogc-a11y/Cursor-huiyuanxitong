...
import { MobileCardList, MobileCard, MobileCardGrid, MobileCardGridItem, MobileCardRow, MobileCardCollapsible, MobilePagination } from "@/components/ui/mobile-data-card";
...
export function OrderTable(props: OrderTableProps) {
  ...
  if (useCompactLayout) {
    return (
      <MobileCardList>
        {orders.length === 0 ? (
          <p className="text-center py-8 text-muted-foreground text-sm">{t("暂无订单数据", "No orders")}</p>
        ) : orders.map((order) => (
          <MobileCard key={order.id} compact className={order.status === "cancelled" ? "opacity-60" : ""}>
            <div className="flex items-center justify-between gap-1.5">
              <div className="flex items-center gap-1.5 overflow-hidden min-w-0">
                <span className="font-mono text-[11px] text-muted-foreground truncate">{order.id.slice(0, 8)}</span>
                <span className="text-[10px] text-muted-foreground shrink-0">{order.createdAt?.replace(/^\d{4}\//, '').slice(0, 11)}</span>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                ...
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onEdit(order)}>
                ...
                <TableRow key={order.id} className={order.status === "cancelled" ? "bg-muted/30" : ""}>
                  ...
                  {columnVisibility.isVisible('profitRate') && (
                    <TableCell className="px-1.5 text-center whitespace-nowrap">
                      ...
                      {order.profitRate > 50 && (
                        <Badge variant="outline" className="ml-1 bg-amber-100 text-amber-700 border-amber-300 text-[8px] px-0.5" title="利润率异常过高，请检查数据">
                          ⚠️
                        </Badge>
                      )}
                    </TableCell>
                  ...
                  {columnVisibility.isVisible('status') && (
                    <TableCell className="px-1.5 whitespace-nowrap text-center">
                      {order.status === "cancelled" ? (
                        <Badge variant="destructive" className="text-[9px] px-0.5">取消</Badge>
                      ) : (
                        <Badge variant="success" className="text-[9px] px-0.5">完成</Badge>
                      )}
                    </TableCell>
                  )}
                  {columnVisibility.isVisible('actions') && (
