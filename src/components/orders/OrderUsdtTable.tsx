...
export function OrderUsdtTable(props: OrderUsdtTableProps) {
  ...
  if (useCompactLayout) {
    return (
      <MobileCardList>
        {orders.length === 0 ? (
          <p className="text-center py-8 text-muted-foreground text-sm">{t("暂无USDT订单数据", "No USDT orders")}</p>
        ) : orders.map((order) => (
                  ...
                  {columnVisibility.isVisible('actions') && (
                    <TableCell className="px-2 whitespace-nowrap sticky right-0 z-10 bg-background shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.15)]">
                      <UsdtOrderRowActions
                        ...
                        onEdit={onEdit}
                        onCancel={onCancel}
                        onRestore={async (dbId) => { await onRestore(dbId); toast.success(t("USDT订单已恢复", "USDT order restored")); return true; }}
                        onDelete={onDelete}
