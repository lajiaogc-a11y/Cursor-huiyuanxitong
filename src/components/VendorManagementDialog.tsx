  ...
  return (
    <>
      <DrawerDetail
        ...
        title={
          <span className={isMobile ? 'text-base' : undefined}>
            {t('卡商管理', 'Vendor Management')} — {vendorName}
          </span>
        }
        description={t('管理提款、余额和变动明细', 'Manage withdrawals, balance and change history')}
        sheetMaxWidth="4xl"
      >
        <div className="flex flex-col h-[min(82vh,760px)] min-h-[360px]">
          <Tabs value={activeTab} onValueChange={handleTabChange} className="flex-1 overflow-hidden flex flex-col min-h-0">
      ...
      <DrawerDetail
        ...
        onOpenChange={(o) => !o && setEditingWithdrawal(null)}
