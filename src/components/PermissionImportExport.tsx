...
export function PermissionImportExport({ onImportComplete }: PermissionImportExportProps) {
  ...
  const handleExport = useCallback(async (role?: AppRole) => {
    try {
      ...
      const { data, error } = await query.order('module_name').order('field_name');
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const currentPermsMap = new Map(
        (currentPerms || []).map(p => [`${p.role}-${p.module_name}-${p.field_name}`, p])
      );
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          ...
          <DropdownMenuItem onClick={() => handleExport('manager')}>
            <Download className="h-4 w-4 mr-2" />
