...
function getCardEnglishPart(card: unknown): string {
  if (!card) return "-";
  let s = String(card).replace(/\s*[（(][^）)]*[）)]\s*/g, "").trim();
  s = s.replace(/[\u4e00-\u9fff\u3000-\u303f\uff00-\uffef]/g, "").trim();
  return s || "-";
}

function getCountryRate(currencyRates: CurrencyRates, country: unknown): number {
    ...
    <div className="space-y-4">
      ...
      <div className="border rounded-lg p-4 bg-muted/30 space-y-3">
        ...
        <div className="text-xs text-muted-foreground">
          {t("上次更新", "Last updated")}: {formatBeijingTime(currencyRates.lastUpdated)}
        </div>
      </div>
