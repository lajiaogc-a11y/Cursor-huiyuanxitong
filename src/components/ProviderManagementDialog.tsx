    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className={`${isMobile ? 'max-w-full w-full h-[95vh]' : 'max-w-5xl h-[85vh]'} overflow-hidden flex flex-col`}>
          <DialogHeader>
            <DialogTitle className={isMobile ? 'text-sm' : ''}>{t('代付商家管理', 'Provider Management')} - {providerName}</DialogTitle>
            <DialogDescription className={isMobile ? 'text-xs' : ''}>{t('管理充值、余额和变动明细', 'Manage recharges, balance and change history')}</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!editingRecharge} onOpenChange={(open) => !open && setEditingRecharge(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('编辑充值记录', 'Edit Recharge')}</DialogTitle>
          </DialogHeader>
          {editingRecharge && (
