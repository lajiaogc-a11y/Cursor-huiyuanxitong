...
export default function ExchangeRateSettingsTab() {
  ...
  const [countryDeleteId, setCountryDeleteId] = useState<string | null>(null);
  ...
  return (
    <SettingsPageContainer>
      ...
      <AlertDialog open={countryDeleteId !== null} onOpenChange={(open) => !open && setCountryDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("确认删除国家", "Delete country?")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                `确定删除「${countries.find((c) => c.id === countryDeleteId)?.name ?? "—"}」吗？使用该国别的汇率配置可能受影响，此操作不可撤销。`,
                `Delete "${countries.find((c) => c.id === countryDeleteId)?.name ?? "—"}"? Rate settings that use this country may be affected. This cannot be undone.`,
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={isSaving}
              onClick={() => {
                const id = countryDeleteId;
                setCountryDeleteId(null);
                if (id) void handleDeleteCountry(id);
              }}
            >
              {t("删除", "Delete")}
            </AlertDialogAction>
