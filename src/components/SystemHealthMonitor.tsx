...
import { countErrorReportsSince } from "@/services/observability/errorReportService";
import {
  pingEmployeesSample,
  getDataTableHeadCount,
  listWebVitalsSince,
  type HealthDataTableName,
} from "@/services/observability/systemHealthService";
...
export default function SystemHealthMonitor() {
  ...
  const checkHealth = async () => {
    ...
    try {
      ...
      const tableDefs: { name: HealthDataTableName; label: string }[] = [
      ...
      const countResults = await Promise.all(
        tableDefs.map(async (td) => {
          try {
            const count = await getDataTableHeadCount(td.name);
            return { ...td, count };
          } catch {
            return { ...td, count: 0 };
          }
      ...
      let errorCount = 0;
      try {
        errorCount = await countErrorReportsSince(yesterday);
      } catch { /* error reports table may not exist */ }

      // Web Vitals (last 24h averages)
      let webVitals: WebVitalStat[] = [];
      try {
        const vitalsData = await listWebVitalsSince(yesterday);
