...
export default function ActivityGiftDialog({ open, onOpenChange, onSuccess }: ActivityGiftDialogProps) {
  ...
  useEffect(() => {
    const loadRates = async () => {
      const rates = await getFinalRates();
      if (rates) {
        setNairaRate(rates.ngnRate || 0);
        setCediRate(rates.ghsRate || 0);
        setUsdtRate(rates.usdtRate || 0);
      }
    };
    if (open) {
      loadRates();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("活动赠送录入", "Activity Gift Entry")}</DialogTitle>
        </DialogHeader>
