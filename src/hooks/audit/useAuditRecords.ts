    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.error('CRITICAL: Audit revert also failed — record may be in inconsistent state:', revertError);
        }
        notify.error(
          revertOk
            ? t('应用修改失败，审核状态已回滚', 'Failed to apply changes, audit status reverted')
            : t('应用修改失败且回滚失败，请联系管理员手动处理', 'Failed to apply changes AND revert failed — contact admin'),
        );
        await queryClient.invalidateQueries({ queryKey: ['audit-records'] });
        return false;
      }
