...
export type OperationLogsTableT = (zh: string, en: string) => string;

export function useOperationLogsTable(options: {
  activeTab: string;
  effectiveTenantId: string | null | undefined;
  userIsAdmin: boolean;
  language: string;
  t: OperationLogsTableT;
}) {
  const { activeTab, effectiveTenantId, userIsAdmin, language, t } = options;
  ...
  const { data: auditLogsPage, isLoading: loading, isError: isErrorLogs } = useQuery({
    ...
    queryFn: async () => {
      return fetchAuditLogsPage(currentPage, OPERATION_LOGS_PAGE_SIZE, {
        module: moduleFilter,
  ...
  const handleDateRangeChange = useCallback((range: TimeRangeType, start?: Date, end?: Date) => {
  ...
  const restorableLogs = useMemo(() => filteredLogs.filter((log) => canRestore(log)), [filteredLogs, canRestore]);
  ...
  const clearFilters = useCallback((includeSearch: boolean) => {
    setModuleFilter("all");
    setOperationFilter("all");
    setOperatorFilter("all");
    setRestoreStatusFilter("all");
    if (includeSearch) setSearchTerm("");
  }, []);
  ...
  return {
    ...
    currentPage,
    setCurrentPage,
    PAGE_SIZE: OPERATION_LOGS_PAGE_SIZE,
    auditLogsPage,
    loading,
    isErrorLogs,
    handleRefresh,
