...
export async function getOrCreateMemberActivity(
  ...
  try {
    ...
    if (fetchError) {
      console.error('Failed to fetch member activity:', fetchError);
      return null;
    }

    if (existing) {
      return existing as MemberActivityData;
    }
    ...
    const { data: created, error: createError } = await supabase
      ...
      .insert({
        ...
        total_gift_ghs: 0,
        total_gift_usdt: 0,
        order_count: 0, // 订单累积次数
      })
      .select()
      .single();

    if (createError) {
      console.error('Failed to create member activity:', createError);
      return null;
    }

    return created as MemberActivityData;
  } catch (error) {
...
export async function getMemberActivityByPhone(
  ...
  try {
    ...
    if (error) {
      console.error('Failed to fetch member activity by phone:', error);
      return null;
    }

    return data as MemberActivityData | null;
  } catch (error) {
...
export async function getPermanentActivityData(
  ...
  try {
    const { data, error } = await supabase
      .from('member_activity')
      .select('total_accumulated_ngn, total_accumulated_ghs, total_accumulated_usdt, total_gift_ngn, total_gift_ghs, total_gift_usdt, accumulated_profit, accumulated_profit_usdt')
      .eq('member_id', memberId)
      .maybeSingle();
    ...
    return {
      accumulatedNgn: data.total_accumulated_ngn || 0,
      accumulatedGhs: data.total_accumulated_ghs || 0,
      accumulatedUsdt: data.total_accumulated_usdt || 0,
      giftNgn: data.total_gift_ngn || 0,
      giftGhs: data.total_gift_ghs || 0,
      giftUsdt: data.total_gift_usdt || 0,
      accumulatedProfit: data.accumulated_profit || 0,
      accumulatedProfitUsdt: data.accumulated_profit_usdt || 0,
    };
  } catch (error) {
...
export async function batchUpdateMemberActivity(params: BatchUpdateParams): Promise<boolean> {
  try {
    ...
    // 🚀 优化：使用 UPSERT 模式，单次操作完成 get-or-create + update
    // 先尝试直接更新（大多数情况下会员活动记录已存在）
    const updateFields: Record<string, any> = {
      updated_at: new Date().toISOString(),
    };

    // 使用 SQL 表达式进行增量更新，避免先查询再更新
    let updateQuery = supabase
      .from('member_activity')
      .update(updateFields)
      .eq('member_id', memberId);

    // 使用 RPC 或直接 SQL 增量更新会更快，但这里用 JS 逻辑模拟
    // 先尝试快速路径：直接用 member_id 更新
    const { data: existingData, error: selectError } = await supabase
      .from('member_activity')
      .select('id, total_accumulated_ngn, total_accumulated_ghs, total_accumulated_usdt, accumulated_profit, accumulated_profit_usdt, order_count')
      .eq('member_id', memberId)
      .maybeSingle();

    if (selectError) {
      console.error('[MemberActivity] Select error:', selectError);
      return false;
    }
    ...
    if (existingData) {
      ...
      if (accumulatedAmount) {
        ...
        if (currency === 'NGN') {
          updateData.total_accumulated_ngn = (existingData.total_accumulated_ngn || 0) + amount;
        } else if (currency === 'GHS') {
          updateData.total_accumulated_ghs = (existingData.total_accumulated_ghs || 0) + amount;
        } else if (currency === 'USDT') {
          updateData.total_accumulated_usdt = (existingData.total_accumulated_usdt || 0) + amount;
        }
      }
      ...
      if (profitAmount !== undefined && profitAmount !== 0) {
        ...
        if (currency === 'USDT') {
          updateData.accumulated_profit_usdt = (existingData.accumulated_profit_usdt || 0) + profitAmount;
        } else {
          updateData.accumulated_profit = (existingData.accumulated_profit || 0) + profitAmount;
        }
      }

      if (incrementOrderCount) {
        updateData.order_count = (existingData.order_count || 0) + 1;
      }
