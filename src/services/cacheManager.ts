...
export function subscribeTableChanges(
  ...
  const channel = supabase
    .channel(`cache-invalidate-${tableName}`)
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: tableName },
      () => {
        console.log(`[CacheManager] Table ${tableName} changed, invalidating cache`);
        clearCache(cacheKey);
        onUpdate?.();
      }
    )
...
export function subscribeCoreTables(onUpdate: () => void): () => void {
  const channel = supabase
    .channel('core-tables-broadcast')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, onUpdate)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'members' }, onUpdate)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'activity_gifts' }, onUpdate)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'shared_data_store' }, onUpdate)
    .subscribe();
