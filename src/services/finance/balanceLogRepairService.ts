...
async function fetchAllCompletedOrders(fields: string) {
  ...
  while (hasMore) {
    ...
    if (error) {
      console.error('[RepairService] Batch fetch error:', error);
      break;
    }
...
async function fetchAllLedgerEntries(accountType: string, sourcePrefix: string) {
  ...
  while (hasMore) {
    const { data, error } = await supabase
      ...
      .select('source_id, account_id, is_active')
    ...
    if (error) {
      console.error('[RepairService] Ledger fetch error:', error);
      break;
    }
    if (data && data.length > 0) {
...
export async function scanMissingVendorLogs(): Promise<MissingLogSummary[]> {
  ...
  const vendors = await import('@/services/giftcards/giftcardsApiService').then(m => m.listVendorsApi());
...
export async function scanMissingProviderLogs(): Promise<MissingLogSummary[]> {
  ...
  const providers = await import('@/services/giftcards/giftcardsApiService').then(m => m.listPaymentProvidersApi());
...
export async function scanOrphanedLogs(): Promise<OrphanedLogSummary[]> {
  ...
  while (hasMore) {
    const { data, error } = await supabase
      .from('ledger_transactions')
      .select('id, source_id, account_type, account_id')
      .eq('source_type', 'order')
      .eq('is_active', true)
      .range(offset, offset + batchSize - 1);
