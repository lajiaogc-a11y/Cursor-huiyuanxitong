...
export async function loadSharedData<T>(dataKey: SharedDataKey): Promise<T | null> {
  ...
  try {
    const { getSharedDataApi } = await import('@/api/data');
...
export async function saveSharedData<T>(dataKey: SharedDataKey, value: T): Promise<boolean> {
  const tenantId = getEffectiveTenantId();
  if (!tenantId) {
    console.warn(`[SharedData] Cannot save ${dataKey}: no tenant_id`);
    return false;
  }
  try {
    const { postSharedDataApi } = await import('@/api/data');
    const ok = await postSharedDataApi(dataKey, value, tenantId);
    if (ok) {
      const cacheKey = getSharedCacheKey(dataKey, tenantId);
...
export async function loadMultipleSharedData(keys: SharedDataKey[]): Promise<Record<string, any>> {
  try {
    ...
    const { getSharedDataBatchApi } = await import('@/api/data');
    const data = await getSharedDataBatchApi(keys as string[], tenantId || undefined);
    const result: Record<string, any> = {};
    Object.entries(data).forEach(([k, v]) => {
      result[k] = v;
      setCache(getSharedCacheKey(k as SharedDataKey, tenantId), v, CACHE_CONFIG.SHARED_DATA_TTL);
    });
    return result;
  } catch (error) {
...
export function clearCache(): void {
  // 只清除共享数据前缀的缓存
  import('@/services/cacheManager').then(({ clearCacheByPrefix, CACHE_KEYS }) => {
