...
function migrateSettings(parsed: any): ActivitySettings {
  ...
  if (Array.isArray(parsed.accumulatedRewardTiers)) {
    ...
    parsed.accumulatedRewardTiers = DEFAULT_ACTIVITY_SETTINGS.accumulatedRewardTiers;
  }

  if (!parsed.referralReward || typeof parsed.referralReward !== "object") {
    parsed.referralReward = { ...DEFAULT_ACTIVITY_SETTINGS.referralReward };
  }
  ...
  if (!parsed.activity2 || typeof parsed.activity2 !== "object") {
  ...
  return {
    ...
    ...parsed,
    referralReward: { ...DEFAULT_ACTIVITY_SETTINGS.referralReward, ...parsed.referralReward },
    activity2: { ...DEFAULT_ACTIVITY_SETTINGS.activity2, ...parsed.activity2 },
    accumulatedRewardTiers: parsed.accumulatedRewardTiers,
  };
}
...
async function syncTiersToDatabase(tiers: AccumulatedRewardTier[]): Promise<void> {
  try {
    await syncActivityRewardTiersToDatabase(tiers);
  } catch (error) {
