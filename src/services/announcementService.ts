...
export async function publishSystemAnnouncementResult(params: {
  ...
  try {
    const { data, error } = await supabase.rpc("publish_system_announcement" as never, {
    ...
    if (error) {
      return fail("UNKNOWN", error.message || "publish_system_announcement failed", "COMMON", error);
    }
...
export async function listSystemAnnouncementsResult(limit = 50): Promise<ServiceResult<SystemAnnouncement[]>> {
  try {
    const { data, error } = await supabase.rpc("list_system_announcements" as never, {
      p_limit: limit,
    } as never);
    if (error) {
      return fail("UNKNOWN", error.message || "list_system_announcements failed", "COMMON", error);
    }
