...
import { createLedgerEntry, createAdjustmentEntry, reverseAllEntriesForSource, AccountType, SourceType } from '@/services/ledgerTransactionService';
import { 
...
export async function syncMemberActivityOnOrderEdit(params: {
  ...
  try {
    ...
    let query = supabase.from('member_activity').select('*');
    if (memberId) {
    ...
    if (fetchError) {
      console.error('[BalanceLogService] Failed to fetch member activity:', fetchError);
      return false;
    }
    ...
    if (updateError) {
      console.error('[BalanceLogService] Failed to update member activity:', updateError);
      return false;
    }
