...
export function validateImportData(
  ...
  for (const col of tableConfig.columns) {
    ...
    labelToKey[col.labelEn.toLowerCase()] = col.key;
  ...
  for (const header of headers) {
    ...
    if (labelToKey[normalizedHeader]) {
      columnMapping[header] = labelToKey[normalizedHeader];
    }
  }
  ...
  if (tableName === 'members') {
    ...
    const allHeadersGarbled = headers.length > 0 && headers.every(h => isGarbledHeader(h));
