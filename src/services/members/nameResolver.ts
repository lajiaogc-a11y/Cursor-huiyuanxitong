...
async function loadCards(): Promise<void> {
  try {
    ...
    try {
      const { listCardsApi } = await import('@/services/giftcards/giftcardsApiService');
      ...
      console.warn('[NameResolver] API load cards failed, fallback to supabase:', apiErr);
    }
    ...
    cardsData.byName.clear();
    (data || []).forEach(card => {
      const info: MerchantInfo = { id: card.id, name: card.name, status: card.status };
      cardsData.byId.set(card.id, info);
      cardsData.byName.set(card.name, info);
    });
    cardsData.initialized = true;
...
async function loadVendors(): Promise<void> {
  try {
    try {
      const { listVendorsApi } = await import('@/services/giftcards/giftcardsApiService');
      ...
      console.warn('[NameResolver] API load vendors failed, fallback to supabase:', apiErr);
    }
    const { data, error } = await supabase
      .from('vendors')
      .select('id, name, status');
    ...
    vendorsData.byName.clear();
    (data || []).forEach(vendor => {
      const info: MerchantInfo = { id: vendor.id, name: vendor.name, status: vendor.status };
      vendorsData.byId.set(vendor.id, info);
      vendorsData.byName.set(vendor.name, info);
    });
    vendorsData.initialized = true;
    setCache(CACHE_KEYS.VENDORS, true, CACHE_CONFIG.ENTITY_DATA_TTL);
...
async function loadPaymentProviders(): Promise<void> {
  try {
    try {
      const { listPaymentProvidersApi } = await import('@/services/giftcards/giftcardsApiService');
      ...
      console.warn('[NameResolver] API load providers failed, fallback to supabase:', apiErr);
    }
    const { data, error } = await supabase
    ...
    (data || []).forEach(provider => {
      const info: MerchantInfo = { id: provider.id, name: provider.name, status: provider.status };
...
export async function getEmployeeNameByIdAsync(employeeId: string | null | undefined): Promise<string> {
  ...
  // 如果缓存中没有，直接查询
  try {
    const { getEmployeeApi } = await import('@/api/employees');
...
// 兼容 useMerchantNameResolver 的导出
export const getCardId = getCardIdByName;
export const resolvePaymentProviderName = resolveProviderName;
export const getPaymentProviderNameById = resolveProviderName;
export const getVendorNameById = resolveVendorName;
export const getCardNameById = resolveCardName;

// 兼容 useNameResolvers 的导出
export const initMerchantNameResolver = initNameResolver;
export const initializeAllNameResolvers = initNameResolver;
