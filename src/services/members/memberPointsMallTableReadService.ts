...
export async function listRedemptionsByMember(memberId: string, limit: number): Promise<RedemptionTableRow[]> {
  ...
  const rows = await dataTableApi.get<RedemptionTableRow[]>(
    "redemptions",
    `select=id,prize_id,mall_item_id,status,created_at,points_used,quantity&member_id=eq.${q}&mall_item_id=not.is.null&order=created_at.desc&limit=${limit}`,
  );
...
export async function fetchPrizeNameMap(prizeIds: string[]): Promise<Map<string, string>> {
  ...
  const prizes = await dataTableApi.get<{ id: string; name: string }[]>(
    "prizes",
    `select=id,name&id=in.(${prizeIds.join(",")})`,
  );
...
export async function fetchMallItemTitleMap(mallItemIds: string[]): Promise<Map<string, string>> {
  ...
  const items = await dataTableApi.get<Array<{ id: string; title?: string; name?: string }>>(
    "member_points_mall_items",
    `select=id,title,name&id=in.(${mallItemIds.join(",")})`,
  );
