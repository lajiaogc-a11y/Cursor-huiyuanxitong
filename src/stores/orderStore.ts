...
import { normalizeCurrencyCode, CurrencyCode } from '@/config/currencies';
...
export function calculateOrderPoints(paidAmount: number, currency: CurrencyCode): number {
...
/** 生成订单显示编号（用于order_number字段，非数据库主键） */
export function generateOrderNumber(): string {
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const prefix = chars[Math.floor(Math.random() * 26)] + 
