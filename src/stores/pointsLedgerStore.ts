...
export async function initializePointsLedgerCache(): Promise<void> {
  ...
  try {
    const data = await apiGet<unknown[]>(
      `/api/data/table/points_ledger?select=*&order=created_at.desc&limit=2000`
    );
    const rows = Array.isArray(data) ? data : [];

    ledgerCache = rows.map(mapDbToEntry);
...
function mapDbToEntry(r: any): PointsLedgerEntry {
  ...
  if (r.transaction_type === 'consumption') {
    ...
    remark = '推荐奖励';
  } else if (String(r.transaction_type || '').toLowerCase() === 'lottery') {
    remark = '抽奖积分';
  }
...
export async function earnReferralPoints(params: ReferralPointsParams): Promise<PointsLedgerEntry | null> {
  ...
  try {
    const inserted = await apiPost<unknown>(`/api/data/table/points_ledger`, {
      data: {
        member_code,
        phone_number: phone,
        order_id: null,
