...
import { query, queryOne, getPool, withTransaction } from '../../database/index.js';
import type { AuthenticatedRequest } from '../../middlewares/auth.js';
import { assertRpcEmployee, effectiveMemberIdForRpc } from './tableConfig.js';
import { syncPointsLog } from '../points/pointsService.js';
...
export async function handleRpcMemberMallRedeemGroup(ctx: RpcCtx): Promise<RpcDispatchResult> {
  ...
  switch (fn) {
    case 'member_get_spin_quota': {
      ...
      const { getMemberSpinQuotaService } = await import('../points/service.js');
      result = await getMemberSpinQuotaService(memberId);
      break;
    }
