#!/usr/bin/env node
/**
 * 执行会员门户 viewingTenantId 迁移
 * 确保后台发布与前端会员端同步
 * 用法: node scripts/run-member-portal-viewing-tenant-migration.mjs
 */
import pg from "pg";
...
async function main() {
  ...
  const migrationFile = "20260404000000_member_portal_settings_viewing_tenant.sql";
  ...
  try {
    ...
    console.log("  - members.tenant_id 已确保存在并回填");
    console.log("  - RPC 已支持 p_tenant_id，后台发布将与前端会员端同步");
  } catch (err) {
