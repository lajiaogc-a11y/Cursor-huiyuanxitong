...
const migrationFile = "20260408000002_error_reports_tracking_alert.sql";
...
async function main() {
  ...
  try {
    ...
    // 兼容部分历史环境：尚未创建 error_reports 表时跳过此迁移，不阻塞整体发布
    if (error?.code === "42P01" && String(error?.message || "").includes("error_reports")) {
      console.warn(`⚠ ${migrationFile} 已跳过：error_reports 表不存在`);
      process.exit(0);
    }
