...
-- Ensure ledger_transactions table exists on Live before functions reference it
...
-- Ensure RLS policies exist (IF NOT EXISTS not available for policies, use DO block)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ledger_transactions' AND policyname = 'ledger_transactions_employee_select') THEN
    CREATE POLICY "ledger_transactions_employee_select" ON public.ledger_transactions FOR SELECT
      USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role) OR has_role(auth.uid(), 'staff'::app_role));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ledger_transactions' AND policyname = 'ledger_transactions_employee_insert') THEN
    CREATE POLICY "ledger_transactions_employee_insert" ON public.ledger_transactions FOR INSERT
      WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role) OR has_role(auth.uid(), 'staff'::app_role));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ledger_transactions' AND policyname = 'ledger_transactions_employee_update') THEN
    CREATE POLICY "ledger_transactions_employee_update" ON public.ledger_transactions FOR UPDATE
      USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'manager'::app_role) OR has_role(auth.uid(), 'staff'::app_role));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ledger_transactions' AND policyname = 'ledger_transactions_admin_delete') THEN
    CREATE POLICY "ledger_transactions_admin_delete" ON public.ledger_transactions FOR DELETE
      USING (has_role(auth.uid(), 'admin'::app_role));
  END IF;
END $$;

-- Now recreate all functions that reference ledger_transactions as return type
-- They will work because the table now exists
...
BEGIN
  UPDATE ledger_transactions SET is_active = false
...
BEGIN
  SELECT * INTO v_original FROM ledger_transactions
