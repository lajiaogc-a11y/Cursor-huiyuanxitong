...
-- Fix 1: Remove plaintext password fallback from verify_employee_login
...
BEGIN
  ...
  WHERE e.username = p_username
    AND e.status = 'active'
    AND e.password_hash LIKE '$2%'
    AND e.password_hash = extensions.crypt(p_password, e.password_hash);
END;
$function$;

-- Fix 2: Remove plaintext password fallback from verify_employee_login_detailed
...
-- Fix 3: Fix shift_handovers RLS - replace public SELECT with employee-only access
DROP POLICY IF EXISTS "All employees can view shift handovers" ON public.shift_handovers;
CREATE POLICY "employees_can_view_shift_handovers"
