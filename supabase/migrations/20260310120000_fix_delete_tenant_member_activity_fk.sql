-- 修复删除租户时 member_activity 外键约束错误
-- 错误：update or delete on table 'members' violates foreign key constraint member_activity_member_id_fkey
-- 原因：删除 members 前需先删除 member_activity（member_activity.member_id -> members.id）
-- 同时处理其他引用 members 的表：activity_gifts, points_ledger, orders.member_id, referral_relations
...
begin
  ...
  -- orders: 先删除该租户的订单（避免后续 members 删除时 orders.member_id FK 冲突）
