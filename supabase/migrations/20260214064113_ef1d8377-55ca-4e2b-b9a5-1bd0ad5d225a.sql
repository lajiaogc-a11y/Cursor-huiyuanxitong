...
-- Fix: capture current balance AFTER deactivating entries, not before
...
BEGIN
  ...
  -- Capture current balance AFTER deactivating so we chain from the correct active entry
...
-- Fix soft_delete_ledger_entry similarly
...
BEGIN
  ...
  -- Then capture current balance from remaining active entries
  SELECT COALESCE(lt.after_balance, 0)
  INTO v_current_balance
  FROM ledger_transactions lt
  WHERE lt.account_type = p_account_type
