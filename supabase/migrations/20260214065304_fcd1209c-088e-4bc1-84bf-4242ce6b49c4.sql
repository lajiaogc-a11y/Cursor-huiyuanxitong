...
-- Drop duplicate text-typed overloads of functions that cause ambiguous RPC calls
-- Keep only the uuid-typed versions

-- Drop text-typed soft_delete_ledger_entry
DROP FUNCTION IF EXISTS public.soft_delete_ledger_entry(text, text, text, text, text, text, text);

-- Drop text-typed reverse_all_entries_for_order  
DROP FUNCTION IF EXISTS public.reverse_all_entries_for_order(text, text, text, text, text, text, text, text);

-- Recreate reverse_all_entries_for_order with SECURITY DEFINER (uuid version currently lacks it)
...
BEGIN
  ...
  -- Derive restore prefix from source prefix
  ...
  -- Deactivate ALL matched entries FIRST
  ...
  -- Capture current balance AFTER deactivating
