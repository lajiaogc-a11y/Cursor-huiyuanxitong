-- 修复删除租户时的 FK 约束错误：
-- 1. tenants.admin_employee_id 引用 employees，需先置空
-- 2. 确保 employees 在删除租户前被正确删除（先解除 tenants 对 employees 的引用）
...
begin
  ...
  -- 关键：先解除 tenants 对 employees 的引用，否则无法删除 employees
  ...
  -- Delete from tenant-scoped tables (including employees)
  for v_tbl in
    select table_name from information_schema.columns
    where table_schema = 'public' and column_name = 'tenant_id'
